        ...
        if ($first_branch_id) {
            ...
            if ($paid_amount > 0) {
                $lastBilling = Billing::whereNotNull('invoice_no')
                    ...
                    ->first();
                $lastpayment = Payment::orderBy('p_id', 'desc')
                    ->first();
                $year = substr(date("y"), -2);
                ...
                if (!$lastpayment) {
                    ...
                    $data = $lastpayment->receipt_no;  // Get last receipt_no
                    $slice = explode("/", $data);
                    $last_receipt_number = isset($slice[1]) ? preg_replace('/[^0-9]/', '', $slice[1]) : 0;
                    $next_receipt_number = (int)$last_receipt_number + 1;
                    $formatted_receipt_number = sprintf("RP%04d", $next_receipt_number);
                ...
                $billing = new Payment([
                    ...
                    'payment_status'    => $paid_amount >= $billingdata->balance_amount ? '1' : '0',
                    'created_by'        => $request->user()->id,
                    'updated_by'        => $request->user()->id,
                ]);
                ...
                if ($billing) {
                    // After saving the billing, update the status of the original billing record
                    $billingdata->paid_amount = $billingdata->paid_amount + $billing->amount;
                    $billingdata->balance_amount = $billingdata->balance_amount - $billing->amount;
                    $billingdata->invoice_no = $billing->invoice_no;
                    // $billingdata->status = 1; // Update status to 'paid'
                    $billingdata->update(); // Save the update
                }
