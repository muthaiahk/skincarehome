...
class CustomerTreatmentController extends Controller
{
    ...
    public function All(Request $request)
    {
        ...
        $cus_treatment = CustomerTreatment::select('treatment.treatment_id', 'branch.branch_name', 'customer_treatment.treatment_auto_id', 'customer_treatment.complete_status', 'treatment.treatment_name', 'treatment_category.tcategory_id', 'treatment_category.tc_name', 'customer_treatment.status', 'customer.customer_id', 'customer.customer_first_name', 'customer_treatment.cus_treat_id', 'customer_treatment.progress', 'customer_treatment.medicine_prefered', 'customer_treatment.remarks', 'customer_treatment.amount', 'customer_treatment.discount', 'customer.branch_id', 'customer_treatment.created_on')
        ...
        if (isset($branch_id) && $branch_id > 0) {
            $cus_treatment = $cus_treatment->where('customer.branch_id', $branch_id);
        }
        $search_input = $request->search_input;
        if (isset($search_input)) {
            $cus_treatment->where(function ($query) use ($search_input) {
                $query->where('customer.customer_first_name', 'LIKE', "%{$search_input}%")
                    ->orWhere('treatment_name', 'LIKE', "%{$search_input}%")
                    ->orWhere('tc_name', 'LIKE', "%{$search_input}%")
        ...
        if ($request->tc_id > 0) {
            $cus_treatment = $cus_treatment->where('customer_treatment.tc_id', $request->tc_id);
        }

        if ($request->t_id > 0) {
            $cus_treatment = $cus_treatment->where('customer_treatment.treatment_id', $request->t_id);
        }

        if ($request->status != '') {
            $cus_treatment = $cus_treatment->where('customer_treatment.progress', (int)$request->status);
        }

        $page = $request->input('page', 1); // Default to page 1
        $limit = $request->input('limit', 10); // Default limit
        // Get the total count for pagination
        $total = $cus_treatment->count();
        $cus_treatment = $cus_treatment->skip(($page - 1) * $limit)->take($limit)->get();
        ...
        foreach ($cus_treatment as $item) {
            ...
            $b = Payment::where('tcate_id', $item->tcategory_id)->where('treatment_id', $item->treatment_id)->where('customer_id', $item->customer_id)->orderBy('payment.p_id', 'desc')->first();
            $p_id = 0;
            if ($status) {
                $invoice = 0; //completed
                $p_id    = $status->p_id;
            ...
            $result[] = [
                ...
                'treatment_auto_id'       => $item->treatment_auto_id,
                ...
                'amount'             => $item->amount,
                'invoice_no'         => $status ? $status->invoice_no : '',
                'discount'           => $item->discount,
                'branch_name'           => $item->branch_name,
                // 'balance'            =>$invoice,
                'balance'            => $b ? $b->balance : $item->amount,
            ];
        }
    ...
    public function Edit($id)
    {
        ...
        $treatment = CustomerTreatment::select('treatment.treatment_id', 'treatment.treatment_name', 'treatment_category.tcategory_id', 'treatment_category.tc_name', 'customer_treatment.status', 'customer.customer_id', 'customer.customer_first_name', 'customer_treatment.cus_treat_id', 'customer_treatment.progress', 'customer_treatment.medicine_prefered', 'customer_treatment.remarks', 'customer_treatment.amount', 'customer_treatment.discount', 'customer_treatment.status', 'customer.customer_last_name', 'customer.customer_phone')
            ->join('treatment_category', 'treatment_category.tcategory_id', '=', 'customer_treatment.tc_id')
            ->join('treatment', 'treatment.treatment_id', '=', 'customer_treatment.treatment_id')
            ->join('customer', 'customer.customer_id', '=', 'customer_treatment.customer_id')
